...
resource "ncloud_network_acl" "develop_public_acl" {
  ...
  vpc_no = ncloud_vpc.develop_vpc.id /* _no => 고유값인 id */
}
...
resource "ncloud_server" "develop_bastion_server" {
  ...
  server_image_product_code = "SW.VSVR.OS.LNX64.UBNTU.SVR2004.B050" /* TERRAFOMR-NCLOUD-DOCS 등에서 확인 */
